"""pydaq backend (was ucla-lapd/read_hdf5_pydaq.py).

Reads LAPD HDF5 files generated by the Python LAPD_DAQ pipeline. Moved here
unchanged in Step 3 of the reorg (wrap, don't rewrite); the unified
``open_lapd``/``LapdRun`` dispatcher in ``data_analysis.io.lapd_hdf5`` delegates
to these functions, and scope-channel reads are delegated to LAPD_DAQ's
``scope_io`` via ``data_analysis.io.scope``. ``ucla-lapd/read_hdf5_pydaq.py`` is
now a thin re-export shim over this module so existing imports keep working.
"""
