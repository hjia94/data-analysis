...
# Dispersion-derivative lookup tables are cached next to this module so they
# resolve regardless of the caller's working directory (the tables are
# regenerated on first use by _generate_dispersion_derivative_table; they are a
# computed cache and intentionally not version-controlled).
_DATA_DIR = os.path.dirname(os.path.abspath(__file__))
_RDWT_FILE = os.path.join(_DATA_DIR, "Sheffield_zprime_rdWT.txt")
_IDWT_FILE = os.path.join(_DATA_DIR, "Sheffield_zprime_idWT.txt")

# The dispersion-derivative tables are immutable within a run, but _Zprime is
# called once per particle species per spectral-density evaluation, so loading
# and interpolating them every call is wasted I/O. Cache the linear
# interpolators on first use, keyed by file path.
_ZPRIME_INTERP_CACHE = {}


def _load_zprime_interpolators(rdWT_file, idWT_file):
    """Return ``(interp_real, interp_imag)`` for the dispersion tables, cached."""
    key = (rdWT_file, idWT_file)
    if key not in _ZPRIME_INTERP_CACHE:
        rdWT = np.loadtxt(rdWT_file, skiprows=1)
        idWT = np.loadtxt(idWT_file, skiprows=1)
        interp_r = interp1d(rdWT[:, 0], rdWT[:, 1], kind='linear',
                            bounds_error=False, fill_value="extrapolate")
        interp_i = interp1d(idWT[:, 0], idWT[:, 1], kind='linear',
                            bounds_error=False, fill_value="extrapolate")
        _ZPRIME_INTERP_CACHE[key] = (interp_r, interp_i)
    return _ZPRIME_INTERP_CACHE[key]
...
class SheffieldThomson:
    ...
    def _generate_dispersion_derivative_table(self, ximin=-10, ximax=10, dx=0.01, N=4):
        ...
        np.savetxt(_RDWT_FILE, rdWT, header="xi  -2*Re(dW/dxi)", comments="")
        np.savetxt(_IDWT_FILE, idWT, header="xi   2*Im(dW/dxi)", comments="")
        _ZPRIME_INTERP_CACHE.pop((_RDWT_FILE, _IDWT_FILE), None)  # drop any stale cache
        print('finished')
    
    def _Zprime(self, xi, rdWT_file=_RDWT_FILE, idWT_file=_IDWT_FILE):
        """
        ...
        rdWT_file : str, optional
            File containing real part of dW/dxi (default: cached beside this module)
        idWT_file : str, optional
            File containing imaginary part of dW/dxi (default: cached beside this module)
        ...
        """
        # Generate tables if they don't exist, then load (cached) interpolators.
        if not os.path.exists(rdWT_file) or not os.path.exists(idWT_file):
            self._generate_dispersion_derivative_table()
        interp_r, interp_i = _load_zprime_interpolators(rdWT_file, idWT_file)
...
if __name__ == '__main__':
    ...
    plt.title(r'Thomson Scattering Form Factor $S(k, \omega)$ vs Scattered Frequency')
    plt.grid(True)
    plt.tight_layout()
    plt.xlim(0, 2)
